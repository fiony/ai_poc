...
Use these steps to run the pipeline on a free Colab GPU runtime:

1. Open a new Colab notebook and set **Runtime → Change runtime type → Hardware
   accelerator → GPU**.
2. In the first cell, install system and Python dependencies, then clone the
   repo:

   ```bash
   !apt-get update -qq && apt-get install -y ffmpeg
   !git clone https://github.com/fiony/ai_poc.git
   %cd ai_poc
   !pip install -e .
   ```

3. Upload your portrait and audio files to Colab (left sidebar → Files →
   Upload) or mount Google Drive if you prefer persistent storage.
4. Run the CLI with your paths; the `--install-deps` flag is optional if you ran
   the install cell above. The `--cache-dir` flag keeps downloaded model assets
   inside the notebook workspace to avoid re-downloading when re-running cells.

   ```bash
   !lipsync-tool /content/portrait.jpg /content/audio.wav /content/output.mp4 \
       --engine sadtalker \
...
5. Download the resulting `/content/output.mp4` from the Files pane or move it
   into your mounted Drive for persistence.
